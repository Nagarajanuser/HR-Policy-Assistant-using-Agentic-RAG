...
def save_to_semantic_cache(state: GraphState, answer: str):
    ...
    try:
        ...
        logger.exception(f"LLM Rewrite Failed : {e}")
...
# --------------------------------------------------
# Hybrid Intent Detection Node  (Rule based + Semantic)
# --------------------------------------------------
...
# --------------------------------------------------
# History-aware query rewriting
# Conversation Memory or Chat Memory
# --------------------------------------------------
FOLLOWUP_PATTERNS = [
    "it",
    "its",
...
def get_chat_history(session_id: str, limit: int = 6):
    ...
    """
    Return the last N chat messages for a session.
    """
...
def history_query_rewrite_node(state: GraphState):
    logger.info(
        "history_query_rewrite_node Reached"
    )
    question = state["question"]
    ...
    # Skip history rewrite when history is too small
    ...
    # No history
    if history.strip() == "":
        logger.info( "query_rewrite_node - No history")
        return {
            "history_question": state["question"]
        }
    ...
    try:
        ...
        logger.exception("Error while processing / history_query_rewrite_node")
        return {
            "history_question": state["question"]
        }
...
# (Rule-Based Query Rewriting + LLM based Query Rewriting + Canonical Query Rewriting
# --------------------------------------------------
QUERY_REWRITE_RULES = {
...
def generate_node(state: GraphState):
    ...
    logger.info( "generate_node : retrieved_docs: %s", len(state.get("reranked_docs", [])))
...
"""
    ...
    if answer not in INVALID_CACHE_RESPONSES:
        logger.info( "saveed to CACHE ")
        save_to_semantic_cache(
            state=state,
            answer=answer
        )
    else:
        logger.info( "CACHE SKIPPED : Invalid answer")
...
builder.add_node(
    "history_query_rewrite",
    history_query_rewrite_node
)
...
# Generate a PNG image of the graph using Mermaid.
#png_data = graph.get_graph().draw_mermaid_png()  
#with open("main_rag_flow.png", "wb") as f:
#    f.write(png_data)

# Generate Mermaid (.md) content
#mermaid_text = graph.get_graph().draw_mermaid()
# Save to a Markdown file
#with open("main_rag_flow.md", "w", encoding="utf-8") as f:
#    f.write("```mermaid\n")
...
def save_chat_session(session_id: str, user_id: int):
    """
    Save a newly created chat session.
    """
    connection, cursor = get_db_cursor()
    query = """
...
def ask_question(request: QuestionRequest):
    ...
    logger.info("Question: %s", request.session_id)
