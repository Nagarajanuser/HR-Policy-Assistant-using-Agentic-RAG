...
# ---------------------------------------------------
class PineconeSemanticCache(BaseCache):  # this class is for semantic cache for store and retrieval of previous answers
    def __init__(self, pinecone_index, embedding, score_threshold=0.90): # i want to pass the vector name and metadata 
        self.index = pinecone_index
        self.embedding = embedding
        self.score_threshold = score_threshold

    def _extract_question(self, prompt: str) -> Optional[str]:
        # Ignore serialized ChatMessage/JSON wrapper calls to avoid duplicate cache saves
        if '"type":' in prompt or "HumanMessage" in prompt or "type='human'" in prompt:
            return None

        # Only cache the generation prompt
        if "You are an AI HR Policy Assistant" in prompt:
            # Extract only the question line after "Question:"
            match = re.search(r"Question:\s*([^\n\r\"\\]+)", prompt, re.IGNORECASE)
            if match:
                return match.group(1).strip()
        return None

    def _extract_user_metadata(self, prompt: str) -> dict:
        user_meta = {}
        for key in ["department", "country", "location", "access_level"]:
            label = key.replace("_", " ")
            # Extract only single-line value, ignoring trailing newlines or escaped characters
            match = re.search(fr"{label}:\s*([^\n\r,\"\\]+)", prompt, re.IGNORECASE)
            if match:
                user_meta[key] = match.group(1).strip()
            else:
                user_meta[key] = ""
        return user_meta

    # lookup for previous answer
    def lookup(self, prompt: str, llm_string: str) -> Optional[Sequence[Generation]]:
        question = self._extract_question(prompt)
        if not question:
            return None  # Bypass cache for classification or other LLM calls

        user_meta = self._extract_user_metadata(prompt)
        filter_conditions = []
        ...
        if results.matches and results.matches[0].score >= self.score_threshold:
            print(f"[CACHE HIT] Reusing cached answer for prompt with matching metadata!")
            return [Generation(text=results.matches[0].metadata["response"])]
        print("[CACHE MISS] Sending query to LLM...")
        return None

    # update by using this we can retun old answer 
    def update(self, prompt: str, llm_string: str, return_val: Sequence[Generation]) -> None:
        question = self._extract_question(prompt)
        if not question:
            return  # Bypass cache update for classification or other LLM calls
        ...
        print(f"[CACHE UPDATED] Saved new answer to cache with metadata: {user_meta}")
    ...
    def clear(self, **kwargs) -> None:
        ...
        try:
            ...
            print("[CACHE CLEARED] Successfully cleared all cache entries.")
        except Exception as e:
            print(f"[CACHE CLEAR] Index was empty or namespace not found.")

set_llm_cache(PineconeSemanticCache(
    pinecone_index=cache_index,
