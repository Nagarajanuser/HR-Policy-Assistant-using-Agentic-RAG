...
# Hybrid Query Rewriting (Rule-Based Query Rewriting and LLM based)
# Hybrid Search
...
QUERY_REWRITE_RULES = {
    ...
    "late coming": "late attendance policy",
    "insurance": "employee insurance policy"
}

def rule_based_query_rewrite(question: str):
    query = normalize_question(question)

    for key, value in QUERY_REWRITE_RULES.items():
        if key in query:
            return value
...
Rewrite the employee question into a clear and complete search query.

Rules:

1. Preserve the original meaning.
2. Expand abbreviations.
3. Resolve incomplete questions.
...
def query_rewrite_node(state: GraphState):
    ...
    # Rule-Based Rewrite
    rewritten = rule_based_query_rewrite(question)
    if rewritten != question:
        print("[RULE REWRITE]", rewritten)
        return {
            "rewritten_question": rewritten
        }

    # Skip LLM for already good queries
    if len(question.split()) >= 5:
        print("Skip LLM for already good queries")
        return {
